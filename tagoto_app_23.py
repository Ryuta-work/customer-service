...
# =====================================================================================
    ...
    if st.session_state.selected_time:
        ...
        day_reservations = df_for_check[df_for_check['日付'] == selected_date.strftime('%Y-%m-%d')]
        is_lunch_session = st.session_state.selected_time.hour < 15
        session_reservations = day_reservations[pd.to_datetime(day_reservations['時間'], format='%H:%M').dt.hour < 15 if is_lunch_session else pd.to_datetime(day_reservations['時間'], format='%H:%M').dt.hour >= 17]
        active_session_reservations = session_reservations[session_reservations['ステータス'] == '予約済み']
        booked_tables = active_session_reservations['席番号'].str.split(', ').explode().unique().tolist() if not active_session_reservations.empty else []
        st.subheader("1階")
        with st.container(border=True):
