    ...
    if st.session_state.active_keypad:
        with st.container(border=True):
            if "name" in st.session_state.active_keypad:
                ...
                if st.session_state.active_keypad in ['pax', 'edit_pax']:
...
def handle_time_selection(selected_slot):
...
if page == '顧客管理':
    ...
    if is_edit_mode:
        ...
        selected_date = st.date_input("日付を選択", label_visibility="collapsed", on_change=handle_date_change)
    ...
    if st.session_state.selected_time:
        ...
        st.markdown("##### 3. 人数を入力")
        pax_key, pax_keypad_target = ("edit_pax", "edit_pax") if is_edit_mode else ("pax", "pax")
        pax_cols = st.columns([0.25, 0.75]) 
        with pax_cols[0]:
            pax_inner_cols = st.columns([3, 1])
            with pax_inner_cols[0]: 
                st.text_input("人数", key=f"{pax_key}_input", on_change=convert_text_callback, args=(f"{pax_key}_input", 'digit'))
            with pax_inner_cols[1]:
                st.markdown("<div style='height: 28px;'></div>", unsafe_allow_html=True)
                st.button("入力", key=f"{pax_key}_keypad_btn", on_click=set_active_keypad, args=(pax_keypad_target,), use_container_width=True)
        
        if st.session_state.active_keypad == pax_keypad_target:
            draw_keypads()
    ...
    if st.session_state.selected_tables:
        ...
        if st.session_state.has_allergies == "有り":
            st.markdown('<p class="red-text">アレルギーの詳細を記入してください</p>', unsafe_allow_html=True)
            st.text_area("アレルギー詳細", key="allergy_details", label_visibility="collapsed")
